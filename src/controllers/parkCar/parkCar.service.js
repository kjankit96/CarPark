const parkCar = require("./parkCar.controller");
...
const parkingCar = async (req, res) => {
  try {
    ...
    console.log(body);
    if (!body.carNo) {
      throw new LotError(204, "Car Number required to Park");
    }
    let transaction = await parkCar(body.carNo);
    if (transaction) {
